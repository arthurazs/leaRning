...
def multiply_voltage(unit: str) -> dec.Decimal:
    match unit:
        case "V":
    ...
    for file in files:
        ...
        with (base / file.name).with_suffix(".csv").open("w") as csv:
            ...
            for timestamp, ia, ib, ic, va, vb, vc in dat.get_analogs(analog_channel_names):
                for unit in units[:3]:
                    assert_current(unit)
                va_converted = va * multiply_voltage(units[3])
                vb_converted = vb * multiply_voltage(units[4])
                vc_converted = vc * multiply_voltage(units[5])
                timestamp_converted = timestamp * (1 if cfg.in_microseconds else 1_000)
                csv.write(f"{timestamp_converted}, {ia}, {ib}, {ic}, {va_converted}, {vb_converted}, {vc_converted}\n")
